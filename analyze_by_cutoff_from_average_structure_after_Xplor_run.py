...
#####################################################
max_dist = 8.5
possibilites_csv_file_name = "runMaxAmbigFive_8.75A_no_DistViol.csv"
pdb_file_name = "MaxAmbigFive_8.75A_ave.pdb"
data_name = "runMaxAmbigFive_8.5A"
xplor_file_name = "runMaxAmbigFive_8.5A.tbl"
#####################################################

tmp_updated_data_file_name = data_name + "_with_cutoff" + str(max_dist) + "A"
updated_data_file_name = tmp_updated_data_file_name + ".csv"
updated_agregated_data_file_name = "aggregated_" + data_name + "_with cutoff_" + str(max_dist) + "A.csv"
...
def find_dist(index_a, name_a, index_b, name_b):
    ...
    if(dist > max_dist):
        ...
        print(f"violation: dist - {dist}, {index_a}/{name_a}-{index_b}/{name_b}")
    return dist
...
def generate_csv_of_xplor_restraints(agg_data):
    for i in range(len(agg_data)):
        ...
        if(ambig_level == 1):
            ...
            #if unambiguous X-X restraint - delete restraint
...
def write_params_file():
    params_file_name = data_name + "_params.txt"
    file_handler = open(params_file_name, "w")
    file_handler.write("cutoff: " + str(max_dist) + "\n")
    file_handler.write("input ave_struct file: " + pdb_file_name + "\n")
    file_handler.write("input possibilites_csv_file_name: " + possibilites_csv_file_name + "\n")   
    file_handler.write("run name: " + data_name + "\n")
    file_handler.write("low restraint: " + str(low_restraint) + "\n")
    file_handler.write("high restraint: " + str(high_restraint) + "\n")
    file_handler.write(f"longer counter: {longer_test_counter}")
    file_handler.close()
...
print(f"longer counter: {longer_test_counter}")