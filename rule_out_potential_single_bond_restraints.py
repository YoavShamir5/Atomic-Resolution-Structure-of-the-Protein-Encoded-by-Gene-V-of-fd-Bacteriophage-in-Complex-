...
input_csv_xplor_file_name = "runE_filtered_by_16A_xplor"
updated_csv_file_name = input_csv_xplor_file_name + "_no_sb.csv"
updated_tbl_file_name = "runMaxAmbigFive_initial_noSB.tbl"
low_restraint = 2
high_restraint = 7.5
...
def filter_csv_data_by_single_bond_correlations():
    for k in range(len(imported_csv_data)):        
        if(any((i[0][0] == i[1][0] and is_single_bond([i[0][1].capitalize(), i[1][1].capitalize()])) for i in imported_csv_data[k][1])):
            print(imported_csv_data[k][1], "\n")
            continue
        else:
